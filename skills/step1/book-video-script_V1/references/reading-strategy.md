...
用于长文档“尽量直接读原文”。长正文用 **Bash** 统计、搜索和按字符预算分窗读入上下文；小文件和阶段产物用 **Read** 精确读取；Python 只用于抽取文本、统计规模和维护 JSON 台账。

## 0. 工具分工

- 长正文：用 Bash 处理，包括规模统计、窗口计划、分窗读取和关键词搜索。
- 小文件：用 Read 处理，包括 skill、references、配置、覆盖台账、修订记录、阶段稿和最终 JSON。
